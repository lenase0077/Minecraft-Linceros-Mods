...
public class PacketHandlers {
    // Removed handleSync
    public static void handleGift(SendGiftPacket payload, ServerPlayNetworking.Context context) {
                ...
                if (data.status() != RelationshipData.Status.NONE && data.partnerUuid().isPresent()) {
                    ...
                    if (partner != null) {
                        ...
                        if (giftStack.has(net.minecraft.core.component.DataComponents.FOOD)) {
                            giftStack.set(com.linceros.couplesmod.init.CouplesComponents.GIFTED_FOOD, true);
                        ...
                        if (partner.getInventory().add(giftStack)) {
                            ...
                            sender.setAttached(CouplesAttachments.RELATIONSHIP, updatedData);
                            ServerPlayNetworking.send(sender, new SyncRelationshipPacket(updatedData.status(), updatedData.partnerUuid(), updatedData.level(), updatedData.xp(), updatedData.lastGiftTime(), updatedData.lastKissTime(), updatedData.startDate(), updatedData.kisses(), updatedData.gifts(), updatedData.sharedBeds()));
                            
                            RelationshipData partnerData = partner.getAttachedOrCreate(CouplesAttachments.RELATIONSHIP);
                            ...
                            partner.setAttached(CouplesAttachments.RELATIONSHIP, updatedPartnerData);
                            ServerPlayNetworking.send(partner, new SyncRelationshipPacket(updatedPartnerData.status(), updatedPartnerData.partnerUuid(), updatedPartnerData.level(), updatedPartnerData.xp(), updatedPartnerData.lastGiftTime(), updatedPartnerData.lastKissTime(), updatedPartnerData.startDate(), updatedPartnerData.kisses(), updatedPartnerData.gifts(), updatedPartnerData.sharedBeds()));
