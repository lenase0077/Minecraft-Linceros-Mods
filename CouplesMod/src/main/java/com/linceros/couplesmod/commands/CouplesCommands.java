...
public class CouplesCommands {
    ...
    private static int setLevel(CommandContext<CommandSourceStack> ctx, int level, ServerPlayer target) {
        RelationshipData data = target.getData(CouplesAttachments.RELATIONSHIP);
        RelationshipData updated = data.withXp(level, data.xp());
        target.setData(CouplesAttachments.RELATIONSHIP, updated);
        sync(target, updated);
    ...
    private static int divorce(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        ...
        if (data.partnerUuid().isPresent()) {
            ...
            if (partner != null) {
                partner.setData(CouplesAttachments.RELATIONSHIP, RelationshipData.EMPTY);
                sync(partner, RelationshipData.EMPTY);
            }
        }
        target.setData(CouplesAttachments.RELATIONSHIP, RelationshipData.EMPTY);
    ...
    private static void sync(ServerPlayer target, RelationshipData data) {
        PacketDistributor.sendToPlayer(target, new SyncRelationshipPacket(data.status(), data.partnerUuid(), data.level(), data.xp(), data.lastGiftTime(), data.lastKissTime(), data.startDate(), data.kisses(), data.gifts(), data.sharedBeds()));
    }
}
